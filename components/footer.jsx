...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-background pt-20 pb-8">
  ...
  [DF]
            ...
            <div className="flex gap-3 mt-6">
              {['Li', 'Ig', 'Gh'].map((label, i) => (
                <motion.a
                  key={i}
                  href="#"
                  className="w-10 h-10 rounded-full border border-ag-border flex items-center justify-center text-ag-body text-xs font-mono hover:bg-ag-accent hover:text-background hover:border-transparent transition-all duration-300"
          ...
          <div>
            <h4 className="font-mono text-label-sm uppercase text-ag-muted mb-6">
              {t('footer.navTitle')}
            </h4>
            <div className="flex flex-col gap-3">
              {[
                ...
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-ag-body hover:text-foreground transition-colors text-sm"
                >
                  {link.label}
                </Link>
              ))}
