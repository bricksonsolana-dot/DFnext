...
export default function ContactPage() {
  ...
  const onSubmit = async (data) => {
    ...
    // Bots fill it instantly
    if (window.__formLoadTime && (Date.now() - window.__formLoadTime) < 3000) {
      setHoneypotTriggered(true);
      setIsSubmitted(true);
      return;
    }
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            ...
            <div className="mt-8 rounded-lg overflow-hidden border border-ag-border aspect-[16/9] bg-ag-surface flex items-center justify-center">
              <div className="text-center">
                ...
                <span className="text-ag-muted text-sm font-mono">{t('contactPage.location')}</span>
              </div>
            </div>
          </motion.div>
              ...
              {isSubmitted ? (
                ...
                >
                  {honeypotTriggered ? (
                    <>
                      {/* ═══════════════════════════════════════════ */}
                      {/* MESSAGE TO THE BOT / HACKER                */}
                      {/* They see "success" but nothing was sent    */}
                      {/* ═══════════════════════════════════════════ */}
                      <motion.div 
                        initial={{ scale: 0 }} 
                        animate={{ scale: 1 }} 
                      ...
                      <p className="text-ag-body">
                        {t('contactPage.form.successText')}
                      </p>
                      {/* 
                        The bot/hacker thinks it worked.
                        In reality, nothing was submitted.
                ...
                <motion.form 
                  ...
                  className="space-y-8"
                  ref={(el) => {
                    // Record when the form loads (timing check)
                    if (el && !window.__formLoadTime) {
                      window.__formLoadTime = Date.now();
                    }
                  }}
                >

                  {/* ═══════════════════════════════════════════════ */}
                  {/* HONEYPOT TRAP #1: Fake field that looks real   */}
                  {/* Hidden from humans via CSS, bots see it as     */}
                  {/* a normal field and fill it in                  */}
                  {/* ═══════════════════════════════════════════════ */}
                  <div 
                    aria-hidden="true" 
                    style={{ 
                  ...
                  <motion.button 
                    ...
                    className="inline-flex items-center gap-2 px-8 py-4 bg-ag-accent text-background font-heading font-semibold rounded-full text-sm disabled:opacity-50 disabled:cursor-not-allowed" 
